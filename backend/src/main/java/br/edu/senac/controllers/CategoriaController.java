...
public class CategoriaController implements IControllerPattern<CategoriaDTO, Long> {
  ...
  public ResponseEntity<CategoriaDTO> post(@RequestBody CategoriaDTO object) {
    var novaCategoria =
        this.categoriaService.insert(modelMapper.map(object, CategoriaEntity.class));
  ...
  public ResponseEntity<CategoriaDTO> put(
      @PathVariable Long categoriaId, @RequestBody CategoriaDTO object) {
    var categoria =
        this.categoriaService.update(categoriaId, modelMapper.map(object, CategoriaEntity.class));
    var categoriaResponse = modelMapper.map(categoria, CategoriaDTO.class);
    return ResponseEntity.ok().body(categoriaResponse);
  }
