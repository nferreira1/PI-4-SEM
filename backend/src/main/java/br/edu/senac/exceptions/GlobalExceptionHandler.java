...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    return ResponseEntity.status(exception.getStatus())
        .body(
            new ErrorResponseException(
                exception.getStatus(), exception.getMessage(), exception.getErrors()));
  }
  ...
  protected ResponseEntity<Object> handleConstraintViolationException(
    ...
    ErrorResponseException errorResponseException =
        new ErrorResponseException(HttpStatus.UNPROCESSABLE_ENTITY, "Erro de validação.");

    for (ConstraintViolation<?> violation : ex.getConstraintViolations()) {
      errorResponseException.addError(
  ...
  protected ResponseEntity<Object> handleDataIntegrityViolationException(
    ...
    ErrorResponseException errorResponseException =
        new ErrorResponseException(HttpStatus.CONFLICT, "Erro de integridade de dados.");
    ...
    for (Throwable cause = ex.getCause(); cause != null; cause = cause.getCause()) {
      ...
      if (message != null && message.contains("Key (")) {
        ...
        if (startIndex > 0 && endIndex > startIndex) {
          ...
          customMessage =
              String.format("O %s '%s' já está cadastrado no sistema.", fieldName, duplicateValue);
        }
  ...
  //        return buildErrorResponse(new ErrorResponseException(HttpStatus.INTERNAL_SERVER_ERROR,
